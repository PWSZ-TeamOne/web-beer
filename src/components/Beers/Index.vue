    ...
    <v-row>
      <v-card class="w-100">
        <v-card-title>
          ...
          <v-btn
            v-if="
              this.$store.state.meetingUserId == this.$store.state.user.userId
            "
            elevation="2"
            class="mx-3"
        ...
        <v-data-table :headers="headers" :items="beers" :search="search">
          ...
          >
            ...
            <v-icon small @click="getRateOfTheBeer(item.id, item.name)">
              mdi-star
            </v-icon>
          </template>
      ...
      <v-dialog v-model="dialog" max-width="600px">
        <v-card>
          <v-card-title>
            <span class="headline"
              >Rate the beer: <strong>{{ this.beerName }}</strong></span
            >
          </v-card-title>
          <v-card-text>
            <v-container>
  ...
  methods: {
    ...
    deleteItem(id) {
      if (confirm("Are you sure you want to delete event?")) {
        let eventsToDelete = db.collection("beers").where("id", "==", id);
    ...
    editItem(id) {
      store.dispatch("setEditedBeerId", id).then(() => {
